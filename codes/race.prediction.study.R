...
ratio_cutoff = 1 # ratio of two largest race membership probabilities, termed as race prediction confidence score.
...
write.table(df.samples.cv,
            file = sprintf("%s/race.prediction.study_pred.csv", out_dir),
            sep = ",",
            row.names = F,
            col.names = T)
