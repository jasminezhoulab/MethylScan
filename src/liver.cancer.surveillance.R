...
cat(c(sprintf("Study: %s", study_id),
      ...
      sprintf("Feature matrix file (row as sample; column as feature): %s", input_feature_matrix_file),
      sep = "\n")
...
########################################################################
## Perform liver-cancer surveillance

## Feature matrix file for liver-cancer surveillance
## Rows are samples and columns are liver-cancer-specific features
## Columns: sample_id, feature1, feature2, ...
if (grepl("\\.gz$", input_feature_matrix_file, ignore.case = TRUE)) {
  df.data <- read.csv(gzfile(input_feature_matrix_file), row.names = 1, check.names = FALSE)
} else {
...
test_samples[names(pred.scores), "pred.score_liver.cancer"] = unname(pred.scores)

# Output prediction scores for liver cancer surveillance
write.table(test_samples, file = out_file, sep = ",", quote = FALSE, row.names = FALSE, col.names = TRUE)

cat(sprintf("Output: %s\n", out_file))
cat("   pred column (last one column): pred.score_liver.cancer\n")
